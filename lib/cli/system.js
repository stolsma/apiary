...
module.exports = function setupConfig(app) {
	...
	app.cli('/start/direct', function start(cmd, tty, end) {
		if (!cmd.connected()) {
			...
			utils.start(cmd.config,	function(err) {
				if (err) {
	...
	app.cli(['/start', '/start/daemon'], function start(cmd, tty, end) {
		if (!cmd.connected()) {
			...
			// daemonize this start script and start child process with Apiary
			utils.startDaemonized(cmd.config);
			
			tty.info('');
			tty.info('The Apiary system is starting as daemon!'.magenta);
			tty.info('');
		} else {
			cmd.logError('Already an Apiary System started!');
			tty.end(1);
