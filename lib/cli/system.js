...
module.exports = function setupConfig(app) {
	...
	app.cli('/start/direct', function start(cmd, tty) {
		...
		utils.start(function(err, result) {
	...
	app.cli(['/start', '/start/daemon'], function start(cmd, tty) {
		...
		utils.startDaemonized(cmd.config.get('system:runningBase'), false);
	...
	app.cli('/stop', function stop(cmd, tty) {
		utils.stop(cmd.config.get('system:runningBase'), function(err, pid) {
			if (err) {
				if (err.code == 'ESRCH') {
	...
	app.cli('/clean', function clean(cmd, tty) {
		utils.clean(cmd.config.get('system:runningBase'), function(err, result){
			if (err) {
