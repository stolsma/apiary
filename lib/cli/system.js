...
module.exports = function setupConfig(app) {
	 ...
	 */
	app.cli('/start/direct', function start(cmd, tty, end) {
		if (!cmd.connected()) {
			tty.info('');
			...
			utils.start(
				cmd.config.get('apiary'),
				cmd.config.get('cli:socket'),
				function(err, cliServer) {
					if (err) {
						cmd.logError('Error starting the Apiary system direct!', err);
						tty.end(1);
						end();
					}
					tty.info('');
					...
					tty.info('');
					// No end() because this process needs to keep running!!! ;-)
				}
			);
		} else {
	 ...
	 */
	app.cli(['/start', '/start/daemon'], function start(cmd, tty, end) {
		if (!cmd.connected()) {
			tty.info('');
			tty.info('Starting the Apiary system!'.magenta);
			
			// deamonize this start script and start child process (with Forever) with Apiary in it
			utils.startDaemonized(path.join(cmd.config.get('cli:base'), '4evr'), false);

			// TODO: check if really started!!!! Probably with timer function to check CLI connection with system
			tty.info('');
			tty.info('The Apiary system is started as daemon!'.magenta);
			tty.info('');
		} else {
			cmd.logError('Already an Apiary System started!');
			tty.end(1);
	...
	app.cli('/stop', function stop(cmd, tty, end) {
		if (cmd.connected()) {
			utils.stop(cmd.client, path.join(cmd.config.get('cli:base'), '4evr'), function(err, pid) {
				if (err) {
					if (err.code == 'ESRCH') {
