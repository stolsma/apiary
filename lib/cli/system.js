...
module.exports = function setupConfig(app) {
	...
	app.cli('/start/direct', function start(cmd, tty, end) {
		if (!cmd.connected()) {
			...
			tty.error('Already an Apiary System started!');
			tty.end(1);
			end();
		}
	});
	...
	app.cli(['/start', '/start/daemon'], function start(cmd, tty, end) {
		if (!cmd.connected()) {
			...
			// TODO: check ik really started!!!! Probably with timer function to check CLI connection with system
	...
	app.cli('/stop', function stop(cmd, tty, end) {
		if (cmd.connected()) {
			utils.stop(cmd.client, cmd.config.get('cli:base'), function(err, pid) {
				if (err) {
					if (err.code == 'ESRCH') {
	...
	app.cli('/clean', function clean(cmd, tty, end) {
		if (!cmd.connected()) {
			...
			tty.error('Can not clean when the Apiary System is running!');
			tty.end(1);
			end();
		}
	});
