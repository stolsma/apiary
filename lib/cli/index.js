...
var base = path.join('/var', 'local', 'apiary'),
	baseSocket = 'socket',
	socketFile = 'apiary.sock',
	apiaryConf = '.apiaryconf',
	configFile = 'apiary.json',
	defaultConfig = {
		apiary: {
			config: {
				file: path.join(base, configFile),
			}
		},
		cli: {
			socket: path.join(base, baseSocket, socketFile),
			base: base,
...
module.exports = function setupConfig(app) {
	...
	app.use(function(cmd, tty, next) {
		...
		// Try to create central location for Apiary config, log and socket files
		base = cmd.flags['l'] || cmd.flags['location'] || base;		
		try {
			fs.mkdirSync(base, 0775);
			fs.mkdirSync(path.join(base, baseSocket), 0775);
		} catch (e) {
			if (e.code !== 'EEXIST') {
				cmd.logError('Error creating service directory.', e);
				process.exit(1);
			}
		}
		
		// TODO: create .apiaryconf or -c/--conf if it doesnt exists
		...
		confFile = cmd.flags['c'] || cmd.flags['conf'] || path.join(process.env.HOME, apiaryConf);
		try {
			if (!path.existsSync(confFile)) {
				fs.writeFileSync(confFile, JSON.stringify(defaultConfig, null, 2));
				fs.chmodSync(confFile, 0660);
				fs.chownSync(confFile, uid, gid);
			}
		} catch (e) {
			cmd.logError('Error creating ' + confFile, e);
		}
