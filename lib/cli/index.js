...
module.exports = function setupConfig(app) {
  ...
  app.use(function(cmd, tty, next) {
    ...
    var uid = parseInt(process.env.SUDO_UID, 10) || 0,
        gid = parseInt(process.env.SUDO_GID, 10) || 0,
      
      //*********************************************************************************************************************
      // ********************* BUG: Process.env.HOME doesnt work when 'sudo' is used. Find solution *************************
      //*********************************************************************************************************************
      cliConfFile = app.cliConfFile = cmd.flags['c'] || cmd.flags['conf'] || path.join(process.env.HOME, apiaryConf);

    try {
      if (!fs.existsSync(cliConfFile)) {
        fs.writeFileSync(cliConfFile, JSON.stringify(utils.createDefaultConfig(cmd.flags['l'] || cmd.flags['location'] || base), null, 2));
        fs.chmodSync(cliConfFile, 660);
  ...
  app.use(function(cmd, tty, next) {
    ...
    utils.createDirs(cmd.config.get('cli'), 0, 0, function() {
      ...
      app.cliConfFile = cmd.config.file.file;
      tty.info('Using CLI config file ' + app.cliConfFile.magenta);
