...
function startController(err, config) {
  ...
  child.on('child::stopresult', function(err) {
    var result = err ? 1 : 0;
//    console.log('(Controller) Apiary stopped!!');
    child.kill();
    process.exit(result);
  });
...
//    console.log('(Controller) SIGINT received!!!!');
    child.emit('child::stop', {});
  });
...
function startMain() {
  ...
  process.parent.on('child::start', function(options) {
    ...
    apiary.start(apiaryOptions, function(err) {
      ...
      apiary.cliServer = CliServer().listen(path.join(options.socket, options.socketFile), function(){});
  ...
  process.parent.on('child::stop', function() {
