...
module.exports = function setupApps(app) {
	...
	app.use(function(cmd, tty, next) {
		try {
			...
			try {
				...
				['repository', 'name', 'domain', 'subdomain', 'env', 'user', 'scripts'].forEach(function(item) {
				...
				tty.info('No deployment script was detected');
				eyes.inspect(e);
			}
			...
			tty.info('Current default application user: ' + app.user.yellow);
			cmd.app = app;
		} 
		catch (e) {
	...
	app.cli(['/apps/clean', '/apps/clean/:appname'], function appsClean(cmd, tty, end) {
		if (cmd.connected()) {
			...
			cmd.client.remote.appClean(app, function(err, result) {
				if (err) {
	...
	app.cli(['/apps/start'], function appsStart(cmd, tty, end) {
		if (cmd.connected()) {
			var app = cmd.app;
			cmd.client.remote.appStart(app, function(err, result) {
				if (err) {
	...
	app.cli(['/apps/stop', '/apps/stop/:appname'], function appsStop(cmd, tty, end) {
		if (cmd.connected()) {
			...
			cmd.client.remote.appStop(app, function(err, result) {
				if (err) {
