...
Controller.prototype.stop = function(cb) {
  ...
  function stopService(name, next) {
    self.stopService(name, function(err) {
      next(null, {
        name: name,
        err: err
      });
...
Controller.prototype.startService = function(options, cb) {
  ...
  child.on('disconnect', this.onDisconnect.bind(this, name));
  child.on('disconnected', this.onDisconnected.bind(this, name));
  child.on('close', this.onClose.bind(this, name));
  ...
  child.on('warn', this.onWarn.bind(this, name));
 ...
 * @param {Function} cb Callback to call when ready with (err)
 */
Controller.prototype.stopService = function(name, timeout, cb) {
  ...
  timeoutId = setTimeout(
    function(){
      var err = new Error('Timeout stopping System Service :' + name);
      self.log('log::' + self.name + '::' + name + '::stop::error', err);
      child.stop();
      timeoutId = null;
      cb(err);
    }, 
  ...
  // actions to execute when the child Service is indicating that it is in a clean state
  // listen for callback info by using a random callback event of 60/6 = 10 characters
  var cbEvent = utils.randomString(60) + '::ready';
  ...
  child.once('close', function() {
    self.log('log::' + self.name + '::' + name + '::childclosed', {});
  ...
  self.log('log::' + self.name + '::' + name + '::stopservice', timeout);
 ...
 *     `uid` this.uid
 */
Controller.prototype.onStart = function(name, childData) {
  this.log('log::' + this.name + '::' + name + '::onstart', childData);
};
 ...
 *     `uid` this.uid
 */
Controller.prototype.onRestart = function(name, childData) {
  this.log('log::' + this.name + '::' + name + '::onrestart', childData);
};
...
Controller.prototype.onStop = function(name, childData) {
  this.log('log::' + this.name + '::' + name + '::onstop', childData);
};
 ...
 */
Controller.prototype.onExit = function onExit(name /* spinning, code, signal */) {
  // announce that this System Service died but still could have open streams!!
  this.log('log::' + this.name + '::' + name + '::onexit', {});
};


/**
 * Called when the Intercom.Child object emits the 'disconnect' event.
 */
Controller.prototype.onDisconnect = function(name, type) {
  // announce that this System Service IPC disconnects
  this.log('log::' + this.name + '::' + name + '::ondisconnect', type);
};
 ...
 * Called when the Intercom.Child object emits the 'disconnect' event.
 */
Controller.prototype.onDisconnected = function(name, type) {
  // announce that this System Service IPC disconnected
  this.log('log::' + this.name + '::' + name + '::ondisconnected', type);
};


/**
 * Called when the Intercom.Child object emits the 'close' event.
 * @param {Boolean} spinning Exited within minimum required uptime (minUptime)
 */
Controller.prototype.onClose = function onClose(name /* spinning, code, signal */) {
  // announce that this System Service died and closed all streams!!
  this.log('log::' + this.name + '::' + name + '::onclose', {});
};
...
/**
 * Called when the child monitor object emits the 'warn' event.
 * @param {Error} data The error object send with error
 */
Controller.prototype.onWarn = function(name, data) {
  this.log('log::' + this.name + '::' + name + '::warn', data.toString());
};
...
Controller.prototype.fork = function(options) {
  var cwd = options.cwd || process.cwd(),
    ...
    childOptions = {
      ...
      spawnWith: {
        uid: options.uid || 0,                  // not described option in NodeJS docs.
        gid: options.gid || options.uid || 0,   // not described option in NodeJS docs.
        detached: true
      }
    };
