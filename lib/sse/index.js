...
Sse.prototype.stop = function(cb) {
	...
	list.forEach(function(name){
		...
		this.ssStop(name, function(err, result) {
			if (err) {
				errList.push(err);
				self.sc.emit('error:sse:stop::' + name, err);
			}
			// last SS is stopped??
...
Sse.prototype.initEvents = function() {
	this.on('sse::startss', this.ssStart.bind(this));
};
...
Sse.prototype.ssStart = function(options, cb) {
	...
	// mechanism. At this moment obly two and three deep events are bridged
	//
	function msgBridge() {
		...
		self.sc.emit.apply(self.sc, [parts.join(this.delimiter)].concat(args));
	}
...
Sse.prototype.onStart = function(name, monitor, childData) {
	this.sc.emit('ss::' + name + '::start', childData.pid, childData);
}
...
Sse.prototype.onRestart = function(name, monitor, childData) {
	this.sc.emit('ss::' + name + '::restart', childData.pid, childData);
}
...
Sse.prototype.onStop = function(name, childData) {
	this.sc.emit('ss::' + name + '::stop', childData.pid, childData);
}
...
Sse.prototype.onExit = function onExit(name, child, spinning) {
	...
	this.sc.emit('ss::' + name + '::exit', child.pid, spinning);
}
...
Sse.prototype.onStdout = function(name, data) {
	this.sc.emit('ss::' + name + '::stdout', data.toString());
}
...
Sse.prototype.onStderr = function(name, data) {
	this.sc.emit('ss::' + name + '::stderr', data.toString());
}
...
Sse.prototype.onError = function(name, data) {
	this.sc.emit('ss::' + name + '::error', data.toString());
}
...
Sse.prototype.spawn = function(options) {
	...
	child = new Child(script, foreverOptions);
