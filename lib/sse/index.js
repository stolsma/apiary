 ...
 *    `socket` {Path} Domain socket description
...
var Sse = module.exports = function(sc, options) {
	...
	// What is the socket this System Service is reacting on
	// and create the socket with the correct ownership!
	this.socket = options.socket || './sse.socket';
	...
	// start listening for System Services
	this.listen(this.socket, this.newService.bind(this));
};
inherits(Sse, Dnode);
...
/**
 * Create the Unix domain socket with the correct credentials
 * @param {Object} Options SSE config options:
 *    `socket` {Path} Domain socket description
 * 	  `uid` {String/Integer} Socket user id, default is -1 (current)
 * 	  `gid` {String/Integer} Socket group id, if not defined the given uid is used, if thats not defined -1 (current)
 */
Sse.prototype.createSocket = function(options) {
}
...
Sse.prototype.ssStart = function(options, cb) {
	...
	var name = options.ssid = options.name = options.name || 'SystemService' + ssBase++;

	// TODO: deepclone options object to prevent interference from other processes
	// options = deepclone(options);
	
	// spawn SS Child
	this.running[name] = child = this.spawn(options);
	...
	// actions to execute when System Service is ready
	this.on('running', function(ssName, conf) {
		if (name === ssName) {
			self.sc.emit('ss::started', {
				'type': options.type,
				'name': ssName,
				'conf': conf
			});
	...
	this.sc.emit('ss::childpid', 'Child PID for System Service ' + options.type + ' is :' + child.child.pid);
...
Sse.prototype.newService = function() {
	console.log('newService is called with: ', arguments);
}
 ...
 *     `uid` this.uid
 */
Sse.prototype.onStart = function(name, monitor, file, childData) {
	this.sc.emit('sse::' + name + '::childstart', file.pid, childData);
}
 ...
 *     `uid` this.uid
 */
Sse.prototype.onRestart = function(name, monitor, file, childData) {
	this.sc.emit('sse::' + name + '::childrestart', file.pid, childData);
}
...
Sse.prototype.onStderr = function(name, data) {
	this.emit('sse::' + name + '::stderr', data.toString());
}
...
Sse.prototype.onError = function(name, data) {
	this.emit('sse::' + name + '::err', data.toString());
}
...
Sse.prototype.spawn = function(options) {
	...
	var cwd = options.cwd || process.cwd(),
		ssid = options.ssid || utils.createUID();
		foreverOptions = {
			silent: true,
			cwd: cwd,
			env: {
	...
	// use forever to keep this System Service running!!
