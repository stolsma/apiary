...
var Apie = module.exports = function() {
	...
	this.apisTypes = {
		'sc_api': path.join(__dirname, '../sc/apis/index.js')
	};
 ...
 * @param {Function} cb Callback to call when ready; will be called with an array of Error (if any)
 */
Apie.prototype.stop = function(cb) {
	...
	list.forEach(function(name){
		...
		this.apisStop(name, function(err, result) {
			if (err) {
				errList.push(err);
				apiary.emit('apis::' + name + '::stop::error', err);
			}
			// last API is stopped??
...
Apie.prototype.initEvents = function() {
	...
	// Activate the message dispatcher to/from the subsystems and the API Services
	this.onAny(function() {
...
Apie.prototype.apisStart = function(options, cb) {
	...
	var child = this.running[name] = this.spawn(options);
	// TODO:  Check if child is created!!!
	...
	// actions to execute when comms with this API Service is ready
	child.on('rpcready', function() {
		// give the new API Service its configuration to initialize with
		child.emit('apie::start', options);
	});

	// actions to execute when the API Service is indicating that its initialized and running
	child.on('child::running', function(monitor, childData) {
		apiary.emit('apis::' + name + '::running', {
			'name': name,
			'type': options.type,
	...
	//
	// TODO: Timer to stop new child when it takes the API Service to long to get initialized
	//
	
	//
	// TODO: Temporary child control message passing, needs to be replaced by other
...
Apie.prototype.apisStop = function(name, timeout, cb) {
	...
	// get API Service child instance
	var child = this.running[name];
	...
	var timeoutId = setTimeout(function(){
			// clear timeout
			timeoutId = null;
			apiary.emit('apis::' + name + '::stoptimer');
			cb(new Error('Timeout stopping API Service :' + name), child);
		}, 
		timeout
	);

	// actions to execute when the API Service is indicating that its ready to be stopped
	child.once('child::stopready', function(data) {
		// API Service is in a clean state. Now stop (i.e. kill) it...
		child.stop();
		delete self.running[name];
		apiary.emit('apis::' + uid + '::stopped', data);
		if (timeoutId) {
