...
CliServer.prototype.appList = function(user, cb) {
	...
	this.apis.emit('sre::' + user + '::app::list', user, null, function(err, result) {
		cb(err, result);
	});
}
...
CliServer.prototype.userAdd = function(user, cb) {
	...
	this.apis.emit('sre::srec::get', user, function(err, sre) {
		...
		self.apis.emit('sre::srec::start', options, function(err, result) {
			cb(err, result);
		});
	});
}
...
CliServer.prototype.userRemove = function(user, cb) {
	...
	this.apis.emit('sre::srec::stop', user, 5000, function(err, sre) {
