 ...
 */
var CliServer = module.exports = function(options) {
...
CliServer.prototype.appStart = function(user, app, cb) {
	...
	apiary.emit('sre::' + user + '::app::start', app, function(err, result) {
		if (!err) {
...
CliServer.prototype.appStop = function(user, app, cb) {
	...
	apiary.emit('sre::' + user + '::app::stop', app, function(err, result) {
		if (!err) {
			// add app to config file
...
CliServer.prototype.appUpdate = function(user, app, cb) {
	...
	apiary.emit('sre::' + user + '::app::update', app, function(err, result) {
		if (!err) {
			// add app to config file
			apiary.config.set('sre:' + user + ':apps:' + app.name, app);
			apiary.config.save();
...
CliServer.prototype.appClean = function(user, app, cb) {
	...
	apiary.emit('sre::' + user + '::app::clean', app, function(err, result) {
		// add app to config file
		apiary.config.clear('sre:' + user + ':apps:' + app.name);
		apiary.config.save();
		cb(err, result);
...
CliServer.prototype.appList = function(user, cb) {
	...
	apiary.emit('sre::' + user + '::app::list', user, null, function(err, result) {
		cb(err, result);
	});
}
...
CliServer.prototype.userAdd = function(user, cb) {
	...
	apiary.emit('sre::srec::get', user, function(err, sre) {
		...
		apiary.emit('sre::srec::start', options, function(err, result) {
			cb(err, result);
		});
	});
}
...
CliServer.prototype.userRemove = function(user, cb) {
	...
	apiary.emit('sre::srec::stop', user, 5000, function(err, sre) {
