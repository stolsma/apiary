...
//  this.apie = apie;
  ...
  this.gid = options.gid || this.uid;
};


/**
 * Attempts to spawn the CLI server for this instance. 
 * @param {String} socket UNIX File Socket to connect to
...
CliServer.prototype.listen = function (socket, cb) {
  ...
  // remove current UNIX File Socket if exists
  if (this.socket && fs.existsSync(this.socket)) {
  ...
  function ready() {
    var err;
    try {
      fs.chownSync(self.socket, self.uid, self.gid);
      fs.chmodSync(self.socket, 0770);
    }
    catch (e) {
      err = e;
    }
    
    self.apis.emit('child::cliserver::ready', {
      uid: self.uid,
      gid: self.gid,
      error: err
    });
  ...
  var err;
  try {
    this.server.listen(this.socket, ready);
  }
  catch (e) {
    err = e;
  }
