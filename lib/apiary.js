...
// catch all non caugth exceptions so we can do something with it from the logs!!
	...
	apiary.init(options, function(err) {
		...
		function startSre(user, next) {
			...
			function startApp(app, next) {
				...
				appOpt.name = app;
				apiary.sc.startApp(user, appOpt, next);
			}
...
apiary.stop = function stop(cb) {
	...
	apiary.sc.stop(function(errList) {
		apiary.sse.stop(function() {
...
apiary.init = function init(options, cb) {
	...
	apiary.config.init(options, function (err) {
		...
		apiary.sse = apiary.Sse(apiary.sc, {
			socket: path.join(apiary.config.get('socket'), 'sse.socket'),
			uid: undefined,
			gid: undefined
		});
		...
		apiary.sse.ssStart({
			...
			apiary.emit('httpproxy start result:', err, result);
		});		
