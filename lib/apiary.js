 ...
 * @param options {Object} SRE startup options: user, port (optional) = apiary.defSrePort, haibuPort (optional) = apiary.defHaibuPort
 * @param cb {Function} Callback function if ready. `err` and `result` are given as arguments to this function 
 */
apiary.startSre = function startSre(options, cb) {
	...
	sre.on('*::carapace::running', function(srecName) {
		sre.emit('srec::start', {
			'haibu-hook-port': haibuPort,
			ready: cb
		});
	});
...
/**
 * Start an App!!
 * @param app, {Object} App definition object: user, etc
 * @param cb {Function} Callback function if ready. `err` and `result` are given as arguments to this function 
 */
apiary.startApp = function startApp(app, cb) {
	...
	this.sreList[app.user].sre.emit('srec::startapp', {
		app: app,
